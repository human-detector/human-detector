...
it('renameGroup() Test 2: return group with unchanged name (empty input)', () => {
  // character limit 30
  const groupObj: Group = new Group('Group 1', 'ID');
  expect(renameGroup('Test group name that will be over the limit', groupObj).groupName).toBe(
    groupObj.groupName
  );
});
...
it('addToGroup Test 1: empty group returns group with 1 camera', () => {
  const groupObj: Group = new Group('Group 1', 'ID');
  const addCamera: Camera = new Camera('0', 'Test camera', 'ID');
...
it('addToGroup() Test 3: Group with cameras added camera', () => {
  ...
  const addCamera = new Camera('0', 'Test Camera', 'ID3');
...
it('addToGroup() Test 4: Group with cameras added camera', () => {
  ...
  groupObj.cameras[0] = new Camera('0', 'Camera 1', 'ID');
  groupObj.cameras[1] = new Camera('0', 'Camera 2', 'ID2');
  groupObj.cameras[2] = new Camera('0', 'Camera 3', 'ID3');
  groupObj.cameras[3] = new Camera('0', 'Camera 4', 'ID4');
  groupObj.cameras[4] = new Camera('0', 'Camera 5', 'ID5');
  groupObj.cameras[5] = new Camera('0', 'Camera 6', 'ID6');
  groupObj.cameras[6] = new Camera('0', 'Camera 7', 'ID7');
  groupObj.cameras[7] = new Camera('0', 'Camera 8', 'ID8');
  const addCamera = new Camera('0', 'Test Camera', 'ID9');
